...
class FullAttention_V11(nn.Module):
    def __init__(self, scale=None, attention_dropout=0.1, output_attention=False, combine_mode="add"):
        super(FullAttention_V11, self).__init__()
        self.scale = scale
        self.output_attention = output_attention
        self.dropout = nn.Dropout(attention_dropout)
        self.combine_mode = combine_mode  # "add" or "mul"
    ...
    def forward(self, queries, keys, values, attn_mask=None, tau=None, delta=None):
        ...
        # 1️⃣ Compute learned attention scores
        scores = torch.einsum("blhe,bshe->bhls", queries, keys)

        # 2️⃣ Combine with externally provided attention
        if attn_mask is not None:
            if self.combine_mode == "add":
                # Treat attn_mask as extra bias (same shape as scores or broadcastable)
...
class Mahalanobis_mask_freq(nn.Module):
    def __init__(self, input_size):
        super(Mahalanobis_mask_freq, self).__init__()
